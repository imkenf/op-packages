...
mwan3_push_update()
{
	...
	# nft. Modifies the 'update' variable in the local scope.
	update="$update"$'\n'"$*";
}

# Helper function to create nftables table and chain if not exists
mwan3_nft_setup()
{
...
mwan3_convert_mark()
{
	local mark_value="$1"
	local mask="$2"
	echo "mark set mark and $mask xor $mark_value"
}
...
mwan3_set_custom_ipset_v4()
{
	...
	for custom_network_v4 in $($IP4 route list table "$1" | awk '{print $1}' | grep -E "$IPv4_REGEX"); do
		LOG notice "Adding network $custom_network_v4 from table $1 to mwan3_custom_v4 set"
		mwan3_push_update "add element ip mwan3 custom_ipv4 { $custom_network_v4 }"
	done
}
...
mwan3_set_custom_ipset_v6()
{
	...
	for custom_network_v6 in $($IP6 route list table "$1" | awk '{print $1}' | grep -E "$IPv6_REGEX"); do
		LOG notice "Adding network $custom_network_v6 from table $1 to mwan3_custom_v6 set"
		mwan3_push_update "add element ip6 mwan3 custom_ipv6 { $custom_network_v6 }"
	done
}
...
mwan3_set_custom_ipset()
{
	...
	# Setup nftables environment
	mwan3_nft_setup ip
	[ $NO_IPV6 -eq 0 ] && mwan3_nft_setup ip6

	# Create nftables sets instead of ipsets
	mwan3_push_update "add set ip mwan3 custom_ipv4 { type ipv4_addr; flags interval; }"
	mwan3_push_update "flush set ip mwan3 custom_ipv4"
	config_list_foreach "globals" "rt_table_lookup" mwan3_set_custom_ipset_v4

	if [ $NO_IPV6 -eq 0 ]; then
		mwan3_push_update "add set ip6 mwan3 custom_ipv6 { type ipv6_addr; flags interval; }"
		mwan3_push_update "flush set ip6 mwan3 custom_ipv6"
		config_list_foreach "globals" "rt_table_lookup" mwan3_set_custom_ipset_v6
	fi

	echo "$update" > "${MWAN3_STATUS_IPTABLES_LOG_DIR}/nft-set_custom_ipset.dump"
	error=$(echo "$update" | $NFT -f - 2>&1) || LOG error "set_custom_ipset: $error"
}
...
mwan3_set_connected_ipv4()
{
	...
	# Setup nftables set
	mwan3_push_update "add set ip mwan3 connected_ipv4 { type ipv4_addr; flags interval; }"
	mwan3_push_update "flush set ip mwan3 connected_ipv4"
	...
	for connected_network_v4 in $cidr_list; do
		mwan3_push_update "add element ip mwan3 connected_ipv4 { $connected_network_v4 }"
	done
	for connected_network_v4 in $candidate_list; do
		mwan3_push_update "add element ip mwan3 connected_ipv4 { $connected_network_v4 }"
	done

	mwan3_push_update "add element ip mwan3 connected_ipv4 { 224.0.0.0/3 }"

	echo "$update" > "${MWAN3_STATUS_IPTABLES_LOG_DIR}/nft-set_connected_ipv4.dump"
	error=$(echo "$update" | $NFT -f - 2>&1) || LOG error "set_connected_ipv4: $error"
}
...
mwan3_set_connected_ipv6()
{
	...
	# Setup nftables set
	mwan3_push_update "add set ip6 mwan3 connected_ipv6 { type ipv6_addr; flags interval; }"
	mwan3_push_update "flush set ip6 mwan3 connected_ipv6"

	for connected_network_v6 in $($IP6 route | awk '{print $1}' | grep -E "$IPv6_REGEX"); do
		mwan3_push_update "add element ip6 mwan3 connected_ipv6 { $connected_network_v6 }"
	done

	echo "$update" > "${MWAN3_STATUS_IPTABLES_LOG_DIR}/nft-set_connected_ipv6.dump"
	error=$(echo "$update" | $NFT -f - 2>&1) || LOG error "set_connected_ipv6: $error"
}
...
mwan3_set_connected_ipset()
{
	...
	# Setup nftables environment and create sets
	mwan3_nft_setup ip
	mwan3_push_update "add set ip mwan3 connected_ipv4 { type ipv4_addr; flags interval; }"
	mwan3_push_update "flush set ip mwan3 connected_ipv4"
	...
	echo "$update" > "${MWAN3_STATUS_IPTABLES_LOG_DIR}/nft-set_connected_ipset.dump"
	error=$(echo "$update" | $NFT -f - 2>&1) || LOG error "set_connected_ipset: $error"
}
...
mwan3_set_dynamic_ipset()
{
	...
	# Setup nftables environment and create sets
	mwan3_nft_setup ip
	mwan3_push_update "add set ip mwan3 dynamic_ipv4 { type ipv4_addr; flags interval; }"
	mwan3_push_update "flush set ip mwan3 dynamic_ipv4"

	if [ $NO_IPV6 -eq 0 ]; then
		mwan3_nft_setup ip6
		mwan3_push_update "add set ip6 mwan3 dynamic_ipv6 { type ipv6_addr; flags interval; }"
		mwan3_push_update "flush set ip6 mwan3 dynamic_ipv6"
	fi

	echo "$update" > "${MWAN3_STATUS_IPTABLES_LOG_DIR}/nft-set_dynamic_ipset.dump"
	error=$(echo "$update" | $NFT -f - 2>&1) || LOG error "set_dynamic_ipset: $error"
}
...
mwan3_set_general_nftables()
{
	local update error family nft_family
	...
	for family in ipv4 ipv6; do
		...
		if ! $NFT list chain $nft_family mwan3 mwan3_hook > /dev/null 2>&1; then
			...
			# Restore conntrack mark
			mwan3_push_update "add rule $nft_family mwan3 mwan3_hook mark and $MMX_MASK == 0x0 ct mark and $MMX_MASK set mark"
			mwan3_push_update "add rule $nft_family mwan3 mwan3_hook mark and $MMX_MASK == 0x0 jump mwan3_ifaces_in"

			# Jump to chain-specific chains
			for chain in custom connected dynamic; do
				mwan3_push_update "add rule $nft_family mwan3 mwan3_hook mark and $MMX_MASK == 0x0 jump mwan3_${chain}_${family}"
			done

			mwan3_push_update "add rule $nft_family mwan3 mwan3_hook mark and $MMX_MASK == 0x0 jump mwan3_rules"
			mwan3_push_update "add rule $nft_family mwan3 mwan3_hook ct mark set mark and $MMX_MASK"

			# Second pass for chain-specific processing
			for chain in custom connected dynamic; do
